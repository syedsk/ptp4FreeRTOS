...
int sk_interface_macaddr(const char *name, struct address *mac)
{
	...
	netif_get_mac(netif, mac->sll.sll_addr);
...
int sk_get_ts_info(const char *name, struct sk_ts_info *sk_info)
{
	...
	memset(sk_info, 0, sizeof(struct sk_ts_info));
	sk_info->valid = 1;
	sk_info->phc_index = 0;  //get_phc_index();
	sk_info->so_timestamping = SOF_TIMESTAMPING_TX_HARDWARE |
				   SOF_TIMESTAMPING_RX_HARDWARE |
				   SOF_TIMESTAMPING_RAW_HARDWARE;

	sk_info->tx_types =  (1 << HWTSTAMP_TX_OFF) | (1 << HWTSTAMP_TX_ON);
	sk_info->rx_filters = (1 << HWTSTAMP_FILTER_NONE) |
	       ...
	       struct address *addr, struct hw_timestamp *hwts, int flags)
{
	...
	struct timespec ts;
	if (flags == MSG_ERRQUEUE) {
		/* no error queue in freertos */
		return -1;
	}
	else
	{
		cnt = lwip_recv_with_ts(fd, buf, buflen, &ts, 0);
