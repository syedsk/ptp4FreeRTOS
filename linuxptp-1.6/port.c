...
int set_tmo_random(int fd, int min, int span, int log_seconds)
{
...
#else
	value_ns = min_ns + (span_ns * (rand() % (1 << 15) + 1) >> 15);
#endif

	#ifdef DEBUG
	printf("%s: min: %d span: %d log_seconds:%d value_ns :%lu\n", __func__,min,span,log_seconds, value_ns);
	#endif
	...
	#ifdef DEBUG
	printf("%s: tmo.it_value.tv_sec: %ld tmo.it_value.tv_nsec: %lu \n", __func__,tmo.it_value.tv_sec, tmo.it_value.tv_nsec);
	printf("%s: tmo.it_value.tv_sec: %ld tmo.it_value.tv_nsec: %lu \n", __func__, (value_ns)/NS_PER_SEC, (value_ns)% NS_PER_SEC);
	#endif
	return timerfd_settime(fd, 0, &tmo, NULL);
}
...
#else
	freeRTOS_clock_gettime(CLOCK_MONOTONIC, &now);
#endif
...
#else
		timer_close(p->fda.fd[FD_ANNOUNCE_TIMER + i]);
#endif
	}
...
enum fsm_event port_event(struct port *p, int fd_index)
{
	...
	if (cnt <= 0) {
		pr_err("port %hu: recv message failed: cnt: %d", portnum(p), cnt);

		pr_err("fd_index %d p->fda.fd: %d %d %d %d\n", fd_index, p->fda.fd[0],
				p->fda.fd[1], p->fda.fd[2], p->fda.fd[3],
				p->fda.fd[4]); 
		msg_put(msg);
		return EV_FAULT_DETECTED;
	}
