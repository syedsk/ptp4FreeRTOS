 ...
 */
struct clock the_clock[PTP_INSTANCES];
#else
...
static int clock_fault_timeout(struct port *port, int set)
{
	...
	if (i.type == FTMO_LINEAR_SECONDS) {
		pr_debug("waiting %ld seconds to clear fault on port %d",
			 i.val, port_number(port));
		return port_set_fault_timer_lin(port, i.val);
	} else if (i.type == FTMO_LOG2_SECONDS) {
		pr_debug("waiting 2^{%ld} seconds to clear fault on port %d",
			 i.val, port_number(port));
			   ...
			   struct default_ds *dds)
{
	...
	c = &the_clock[instance];
#else
	c = &the_clock;
#endif
	udsif = &c->uds_interface;

#ifndef __FREERTOS__
	clock_gettime(CLOCK_REALTIME, &ts);
	srandom(ts.tv_sec ^ ts.tv_nsec);
#else
	freeRTOS_clock_gettime(1, &ts);
...
#else
	cnt = freertos_poll(c->pollfd, (c->nports + 1) * N_CLOCK_PFD, -1);
#endif
	if (cnt < 0) {
		if (EINTR == errno) {
			return 0;
