...
/* support two ptp4l instances in an RTOS env
 * TODO make this locally allocated
 */
static struct default_ds ptp4l_dds[PTP_INSTANCES];
...
int ptp_main(int argc, char *argv[], int instance, char *buf)
{
	...
	struct config *cfg;
	struct default_ds *dds = &ptp4l_dds[instance];
	struct defaultDS *ds = &ptp4l_dds[instance].dds;
	...
	/* store the ptp4l instance in tls */
	vTaskSetThreadLocalStoragePointer(NULL, TLS_INDEX_INSTANCE, (void *) instance);
	...
	/* default to gPTP  cfg */
	config_set_int(cfg, "delay_mechanism", DM_P2P);
	config_set_int(cfg, "network_transport", TRANS_IEEE_802_3);
	config_set_int(cfg, "time_stamping", TS_HARDWARE);
	...
	for ( i = 1; i < argc; i++)
	{
		...
		{
			if (!config_create_interface(argv[i+1], cfg))
				goto out;
		}
		else if ( !strcmp(argv[i], "-s") )
		{
			...
			if (get_arg_val_i('l', argv[i+1], &print_level,
	...
	/* getopt is not re-entrant
	 * the re-entrant version getopt_r is broken on armr5 toolchain */
	#if 0
	while (EOF != (c = getopt_r(argc, argv, "AEP246HSLf:i:p:sl:mqvh", &gopt_reent))) {
		switch (c) {
		case 'A':
	...
	clock = clock_create(cfg, phc_index, &cfg->interfaces, &ptp4l_dds[instance]);
	if (!clock) {
